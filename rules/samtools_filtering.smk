...
rule samtools_sort:
    ...
    group: "igFinder"
    shell:
        "samtools sort -t /tmp -n -m 2000M -@ {threads} -o {output} {input} 1>{log} 2>&1"
...
rule samtools_fastq:
    ...
    shell:
        ...
        "-1 {output.one} -2 {output.two} -s {output.singleton} {input} 1>{log} 2>&1 && "
...
rule count_reads:
    ...
    threads: 4
    shell:
        "samtools view -@ 16 -c {input} > {output} 2>{log} "